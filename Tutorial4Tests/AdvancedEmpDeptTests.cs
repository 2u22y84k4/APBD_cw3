...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeesWithManagers()
    {
        ...
        var withMgr = emps.Where(e=> e.Mgr is not null); 
    ...
    // SQL: SELECT E1.EName AS Emp, E2.EName AS Manager
    //      FROM Emp E1
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            .Join(depts,
                e => e.DeptNo,
                d => d.DeptNo,
                (e, d) => new { e.EName, d.DName, e.Sal }
            )
            .SelectMany(ed => grades
                    .Where(s => ed.Sal >= s.Losal && ed.Sal <= s.Hisal)
                    .Select(s => new { ed.EName, ed.DName, s.Grade }) 
            );
